...
int main(void)
{
    ...
    //LED pins Output
    PyHal_GPIO_SetDir(18,1);   // blue led
    PyHal_GPIO_SetDir(21,1);   //green led 
    
    //input pins
    PyHal_GPIO_SetDir(1,0);  // x
    PyHal_GPIO_SetDir(2,0);  // y
    int x, y,lhs,rhs;	
    while(1)
    {
        x = PyHal_GPIO_Get(1); //taking input x
        y = PyHal_GPIO_Get(2);	//taking input y
        lhs = x|(!y);
	rhs = (x&y) |(x&(!y)) | ((!x) &(!y));
	
	// blue led for lhs, green led for rhs
        if (lhs == 0)
        PyHal_GPIO_Set(18, 0);
        else
        PyHal_GPIO_Set(18, 1);
